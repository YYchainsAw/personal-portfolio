...
6. `400` attaches the generic validation problem to a form error summary and focuses it; `429` shows a localized retry-later message using a bounded Retry-After value; network/5xx failures show a retry button and retain every entered value;
...
Use explicit states `IDLE | SUBMITTING | SUCCEEDED | VALIDATION_ERROR | RATE_LIMITED | RETRYABLE_ERROR`. A new submit aborts an obsolete controller, unmount aborts without showing failure, and a retry reuses the preserved model. On success, announce acceptance and move focus to the success status. On `400`, render only safe common-problem guidance; the server does not echo field values. On `429` and network/5xx, keep inputs and privacy acceptance intact.
