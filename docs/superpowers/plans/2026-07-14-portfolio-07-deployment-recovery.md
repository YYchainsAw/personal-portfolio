...
- secret generation, file permissions, COS/SMTP/rclone least privilege, separate short-lived COS lifecycle-operations credentials, idempotent lifecycle installation during bucket provisioning, independent read-only verification during every preflight, Local staging-scheduler activation evidence, and off-server key custody;
