...
function isKnownPayload(block: PublicBlock): true {
  switch (block.payload.type) {
    ...
    case 'LINK':
      return true
    default:
      return assertExhaustivePayload(block.payload)
  }
}
    ...
    .filter((block) => {
      if (block.type !== block.payload.type) {
        if (import.meta.env.DEV)
          throw new Error(`Published block type mismatch: ${block.type} / ${block.payload.type}`)
        return false
      }
...
<template>
  <div
    v-for="(block, blockIndex) in ordered"
    :key="block.id"
    class="content-block"
    :class="[
    ...
    :data-block-index="String(blockIndex + 1).padStart(2, '0')"
  >
    ...
    >
      <figure
        v-for="(media, mediaIndex) in block.payload.media"
        :key="`${media.assetId}:${media.variant}:${mediaIndex}`"
      >
    ...
    <figure v-else-if="block.payload.type === 'VIDEO'" class="block-video">
      <ResponsiveMedia
        v-if="block.payload.cover"
        :media="block.payload.cover"
      ...
      <p v-else>
        {{
          locale === 'zh-CN' ? '此视频暂时无法安全播放。' : 'This video cannot be played safely.'
        }}
      </p>
    ...
    <figure v-else-if="block.payload.type === 'CODE'" class="block-code">
      <figcaption v-if="block.payload.title">
        {{ block.payload.title }} <small>{{ block.payload.language }}</small>
      </figcaption>
      <pre
        :class="{ 'has-line-numbers': block.payload.showLineNumbers }"
      ><code v-if="block.payload.showLineNumbers" class="code-lines"><span v-for="(line, lineIndex) in codeLines(block.payload.code)" :key="lineIndex" :data-line="lineIndex + 1">{{ line }}</span></code><code v-else>{{ block.payload.code }}</code></pre>
      <p v-if="block.payload.description">{{ block.payload.description }}</p>
    </figure>

    <blockquote v-else-if="block.payload.type === 'QUOTE'">
      <p>{{ block.payload.quote }}</p>
      <cite>{{ block.payload.source }}</cite>
    </blockquote>
    ...
    <a
      ...
      class="block-action block-action--download"
      :href="safeHref(block.payload.href)!"
      data-analytics-type="DEMO_DOWNLOAD"
    >
      <span class="block-action__icon" aria-hidden="true"
        ><DownloadSimple :size="25" weight="regular"
      /></span>
      <span class="block-action__copy">
        <strong>{{ block.payload.label }}</strong>
        <span>{{ block.payload.description }}</span>
        <small
          ...
          >{{ mimeLabel(block.payload.mimeType) }} ·
          {{ formatBytes(block.payload.byteSize) }}</small
        >
      </span>
      <ArrowUpRight class="block-action__arrow" :size="22" weight="bold" aria-hidden="true" />
    </a>

    <a
      v-else-if="block.payload.type === 'LINK' && safeHref(block.payload.href)"
      class="block-action block-action--link"
      :href="safeHref(block.payload.href)!"
