...
const locale = computed<Locale>(() =>
  isLocale(route.params.locale) ? route.params.locale : 'zh-CN',
)
const slug = computed(() => (typeof route.params.slug === 'string' ? route.params.slug : ''))
const key = computed(() => `${locale.value}:${slug.value}`)
const resource = useAsyncRouteData(key, (signal) =>
  publicContentStore.loadProject(locale.value, slug.value, signal),
)
...
watch(
  ...
  (content) => {
    ...
    const cover =
      content.catalog.find((card) => card.projectId === content.project.projectId)?.cover ||
      content.project.media[0] ||
      null
    applySeo(
      {
        kind: 'project',
    ...
    const event = {
      ...
      projectId: content.project.projectId,
      locale: locale.value,
...
<template>
  ...
  >
    <ProjectDetailView
      v-if="resource.value.value"
      :locale="locale"
      :site="resource.value.value.site"
      :project="resource.value.value.project"
      :catalog="resource.value.value.catalog"
    />
  </AsyncState>
</template>
