...
for command_name in cmp date docker find grep install jq mktemp mv rm sha256sum sort stat tar tr unzip zstd; do
  require_command "$command_name"
done
...
while IFS= read -r line || [[ -n "$line" ]]; do
  [[ "$line" != *$'\r'* ]] || die "image lock must use LF line endings"
  [[ -z "$line" || "$line" == \#* ]] && continue
  [[ "$line" =~ ^([A-Z_]+)=(.+)$ ]] || die "invalid image lock entry"
  key="${BASH_REMATCH[1]}"
  value="${BASH_REMATCH[2]}"
  case "$key" in
    NODE_IMAGE|JAVA_BUILD_IMAGE|JAVA_RUNTIME_IMAGE|POSTGRES_IMAGE)
      [[ "$value" =~ ^[A-Za-z0-9][A-Za-z0-9._/-]*:[A-Za-z0-9][A-Za-z0-9._-]*$ ]] ||
        die "invalid reviewed image tag for $key"
      ;;
    UBUNTU_APT_SNAPSHOT)
      [[ "$value" =~ ^20[0-9]{6}T[0-9]{6}Z$ ]] || die "invalid Ubuntu APT snapshot identifier"
      ;;
    TARGET_PLATFORM)
      [[ "$value" == "linux/amd64" ]] || die "production target platform must be linux/amd64"
      ;;
    *) die "unknown image lock key: $key" ;;
  esac
  [[ -z "${locked_images[$key]+x}" ]] || die "duplicate image lock key: $key"
  locked_images[$key]="$value"
done <"$source_dir/deploy/image-lock.env"

for key in NODE_IMAGE JAVA_BUILD_IMAGE JAVA_RUNTIME_IMAGE POSTGRES_IMAGE UBUNTU_APT_SNAPSHOT TARGET_PLATFORM; do
  [[ -n "${locked_images[$key]:-}" ]] || die "missing image lock key: $key"
done
...
resolve_image_digest() {
  local tag="${1:?tag required}"
  local repository candidate candidate_repository selected=""
  local -a candidates=()
  repository="$(repository_from_tag "$tag")"
  note "pulling reviewed base tag $tag"
  docker pull --platform "$target_platform" "$tag" >&2
  mapfile -t candidates < <(docker image inspect "$tag" |
    jq -r '.[0].RepoDigests[]? // empty' | sort -u)
  ((${#candidates[@]} > 0)) || die "Docker did not report a repository digest for $tag"
  for candidate in "${candidates[@]}"; do
    [[ "$candidate" =~ @sha256:[0-9a-f]{64}$ ]] || continue
    candidate_repository="${candidate%@*}"
    if [[ "$candidate_repository" == "$repository" ||
          ("$repository" != */* && "$candidate_repository" == "docker.io/library/$repository") ||
          ("$repository" == "docker.io/library/"* && "$candidate_repository" == "${repository#docker.io/library/}") ]]; then
      [[ -z "$selected" ]] || die "multiple repository digests matched reviewed tag $tag"
      selected="$candidate"
    fi
  done
  [[ -n "$selected" ]] || die "no repository digest matched reviewed tag $tag"
  printf '%s\n' "$tag@${selected#*@}"
}

require_locked_digest_ref() {
  local reference="${1:?digest reference required}"
  local locked_tag="${2:?locked tag required}"
  local digest="${reference#"$locked_tag"@sha256:}"
  [[ "$reference" == "$locked_tag@sha256:$digest" && "$digest" =~ ^[0-9a-f]{64}$ ]] ||
    die "recorded rebuild input does not match locked tag $locked_tag"
}
...
if [[ -n "${PORTFOLIO_REBUILD_FROM_RELEASE_JSON:-}" ]]; then
  ...
  require_locked_digest_ref "$node_image_ref" "${locked_images[NODE_IMAGE]}"
  require_locked_digest_ref "$java_build_image_ref" "${locked_images[JAVA_BUILD_IMAGE]}"
  require_locked_digest_ref "$java_runtime_image_ref" "${locked_images[JAVA_RUNTIME_IMAGE]}"
  require_locked_digest_ref "$postgres_image_ref" "${locked_images[POSTGRES_IMAGE]}"
  for recorded_ref in "$node_image_ref" "$java_build_image_ref" "$java_runtime_image_ref" "$postgres_image_ref"; do
    note "pulling recorded immutable rebuild input $recorded_ref"
    docker pull --platform "$target_platform" "$recorded_ref" >&2
  done
else
  node_image_ref="$(resolve_image_digest "${locked_images[NODE_IMAGE]}")"
  java_build_image_ref="$(resolve_image_digest "${locked_images[JAVA_BUILD_IMAGE]}")"
  java_runtime_image_ref="$(resolve_image_digest "${locked_images[JAVA_RUNTIME_IMAGE]}")"
  postgres_image_ref="$(resolve_image_digest "${locked_images[POSTGRES_IMAGE]}")"
fi
[[ "$postgres_image_ref" =~ ^postgres:17-bookworm@sha256:[0-9a-f]{64}$ ]] ||
  die "reviewed PostgreSQL tag did not resolve to postgres:17-bookworm@sha256"
...
maven_verify_mode="${PORTFOLIO_MAVEN_VERIFY_MODE:-container}"
case "$maven_verify_mode" in
  container)
    docker_socket="${DOCKER_SOCKET:-/var/run/docker.sock}"
    [[ "$docker_socket" == /* && "$docker_socket" != *$'\n'* && "$docker_socket" != *$'\r'* ]] ||
      die "DOCKER_SOCKET must be an absolute Unix socket path"
    [[ -S "$docker_socket" ]] ||
      die "Maven verification needs Docker socket access; set PORTFOLIO_MAVEN_VERIFY_MODE=host on an approved Java 17 runner"
    docker_socket="$(realpath -e -- "$docker_socket")"
    note "running Maven verification in an approved Docker-socket runner"
    docker run --rm \
      --platform "$target_platform" \
      --mount "type=bind,src=$source_dir/backend-parent,dst=/src/backend-parent" \
      --mount "type=bind,src=$docker_socket,dst=/var/run/docker.sock" \
      --add-host host.testcontainers.internal:host-gateway \
      --env TESTCONTAINERS_HOST_OVERRIDE=host.testcontainers.internal \
      --env "SOURCE_DATE_EPOCH=$source_date_epoch" \
      --workdir /src/backend-parent \
      "$java_build_image_ref" ./mvnw -B \
        -Dproject.build.outputTimestamp="$source_date_epoch" verify >&2
    ;;
  host)
    require_command java
    note "running Maven verification on the approved host runner"
  ...
  buildkit)
    ...
    ;;
  *)
    die "PORTFOLIO_MAVEN_VERIFY_MODE must be container, host, or buildkit"
    ;;
esac
...
install -d -m 0750 -- "$staging_dir/ops" "$staging_dir/images"
...
while IFS= read -r -d '' path; do
  ...
  case "$mode" in
    100644) output_mode=0640 ;;
    100755) output_mode=0750 ;;
...
jq -n \
  ...
  '{releaseId:$releaseId,gitCommit:$gitCommit,manifestSha256:$manifestSha256,
    ...
    buildInputs:{nodeImageRef:$nodeImageRef,javaBuildImageRef:$javaBuildImageRef,
